...
dotnet run --no-build --no-launch-profile -c Release --project src/main/Yardarm.CommandLine -- \
    restore -n TestSTJ -x src/main/Yardarm.SystemTextJson/bin/Release/net8.0/Yardarm.SystemTextJson.dll -f netstandard2.0 net6.0 net8.0 --intermediate-dir ./obj/
dotnet run --no-build --no-launch-profile -c Release --project src/main/Yardarm.CommandLine -- \
    generate --no-restore -n TestSTJ -x src/main/Yardarm.SystemTextJson/bin/Release/net8.0/Yardarm.SystemTextJson.dll -f netstandard2.0 net6.0 net8.0 --embed --intermediate-dir ./obj/ --nupkg ./bin/ -v 1.0.0 -i ./bin/mashtub.json
