...
for extension in "Yardarm.SystemTextJson" "Yardarm.NewtonsoftJson"; do
    ...
    restore -n TestSTJ -x $jsonext $httpext -p ExternallyDiscriminatedUnions=true -f netstandard2.0 net8.0 net9.0 net10.0 --intermediate-dir ./obj/

    dotnet run --no-build --no-launch-profile -c Release --project src/main/Core/Yardarm.CommandLine -- \
    generate --no-restore -n TestSTJ -x $jsonext $httpext -f netstandard2.0 net8.0 net9.0 net10.0 --embed --intermediate-dir ./obj/ --nupkg ./bin/ -v 1.0.0 -i ./bin/mashtub.json
